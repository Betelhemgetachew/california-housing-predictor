...
====================================================
Betelhem Kebede | CS-DA03-26035 | Cyber Shujaa Program

How to run:
    1. Make sure you have the saved model in the same directory:
...
If you do NOT have the .pkl file yet, the app will train and save it
automatically on first launch using the California Housing dataset.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# CUSTOM CSS — dark teal + amber accent, monospace headers
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
/* ── Google Fonts ── */
...
.stSlider [class*="thumb"] { background: var(--teal) !important; }
.stSlider [class*="track"] { background: var(--teal-dark) !important; }
...
/* ── Prediction card ── */
.pred-card {
    background: linear-gradient(135deg, #0e2a45, #0d3b5e);
...
# ─────────────────────────────────────────────────────────────────────────────
# HELPER: apply dark style to matplotlib figures
# ─────────────────────────────────────────────────────────────────────────────
BG       = "#0d1b2a"
SURFACE  = "#132233"
SURFACE2 = "#1a2e44"
...
def load_or_train_model():
    """Load saved pipeline or train a fresh one."""
    if os.path.exists(MODEL_PATH):
        with open(MODEL_PATH, "rb") as f:
            model = pickle.load(f)
        return model, "loaded"
    else:
        # Train a quick model if no .pkl file found
        ...
        pipeline = Pipeline([("preprocessor", preprocessor), ("knn", KNeighborsRegressor())])
        ...
        return best_model, "trained"


@st.cache_data(show_spinner=False)
def load_dataset():
    """Load and split the dataset; compute test-set metrics."""
    X, y = fetch_california_housing(return_X_y=True, as_frame=True)
...
best_params = {
    "n_neighbors": knn_step.n_neighbors,
    "weights":     knn_step.weights,
    "p":           knn_step.p,
}
...
with st.sidebar:
    ...
    st.markdown(
        "<p style='color:#7a97b5; font-size:0.8rem;'>Adjust the sliders to describe "
        "the block group, then click Predict.</p>",
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    st.markdown("---")
    st.markdown(
        "<p style='color:#7a97b5; font-size:0.75rem;'>"
        "Target variable: <b>Median House Value</b> in $100,000 units.<br>"
        "Dataset: 1990 California Census · 20,640 samples.</p>",
        unsafe_allow_html=True,
    )
...
# TABS
# ─────────────────────────────────────────────────────────────────────────────
tab_pred, tab_perf, tab_data, tab_about = st.tabs([
    "🏡 Prediction",
    "📊 Model Performance",
    "🔬 Data Insights",
    "ℹ️ About",
    ...
    with col_res:
        ...
        if predict_btn:
            ...
            st.markdown(
                f"""
                <div class="pred-card">
                  <div class="pred-value">${price_usd:,.0f}</div>
                  <div class="pred-label">Estimated Median House Value</div>
                  <div style="color:#00b4d8; font-family:'Space Mono',monospace;
                              font-size:1.1rem; margin-top:0.8rem;">
                    {prediction:.4f} × $100k units
                  </div>
                </div>
                """,
                unsafe_allow_html=True,
            ...
            if prediction < 1.0:
                tier, col = "Budget", "#2ec4b6"
            elif prediction < 2.0:
                tier, col = "Affordable", "#00b4d8"
            elif prediction < 3.5:
                tier, col = "Mid-Range", "#f4a261"
            elif prediction < 5.0:
                tier, col = "Premium", "#e76f51"
            else:
                tier, col = "Luxury", "#e63946"

            st.markdown(
                f"<div style='text-align:center; color:{col}; font-family:Space Mono,monospace;"
                f" font-size:0.95rem; margin-top:-0.5rem;'>▸ Market Tier: <b>{tier}</b></div>",
                unsafe_allow_html=True,
            )

            # Radar-style bar chart for inputs
            st.markdown("#### How your inputs compare to dataset averages")
            means  = X.mean()
            ratios = {f: (user_inputs[f] - X[f].min()) / (X[f].max() - X[f].min())
                      for f in FEATURES}
            ...
            bars = ax.barh(
                list(ratios.keys()),
                list(ratios.values()),
                color=[TEAL if v >= 0.5 else AMBER for v in ratios.values()],
                height=0.55,
                edgecolor="none",
            )
            ax.set_xlim(0, 1)
            ax.set_xlabel("Normalised value (0 = min, 1 = max)", fontsize=8)
            ax.axvline(0.5, color=MUTED, linestyle="--", linewidth=0.8, alpha=0.6)
            ax.set_title("Feature Percentile (relative to dataset range)", fontsize=9, color=TEXT)
            ax.invert_yaxis()
            fig.tight_layout()
            st.pyplot(fig)
            plt.close(fig)
...
with tab_perf:
    ...
    st.markdown(
        "<p style='color:#7a97b5; font-size:0.85rem;'>Evaluated on 20% held-out test set "
        "(4,128 samples).</p>",
        unsafe_allow_html=True,
    )
    ...
    m4.metric("Test Samples","4,128")

    st.markdown("---")
    st.markdown("### Best Hyperparameters (from GridSearchCV)")
    pc1, pc2, pc3 = st.columns(3)
    pc1.metric("n_neighbors",  str(best_params["n_neighbors"]))
    pc2.metric("weights",      str(best_params["weights"]))
    ...
    with col_a:
        ...
        lims = [min(y_test.min(), y_pred_all.min()) - 0.1,
                max(y_test.max(), y_pred_all.max()) + 0.1]
        ...
        ax.set_title(f"Actual vs Predicted  (R² = {r2:.4f})", color=TEXT, fontsize=10)
    ...
    with col_b:
        ...
        ax.set_ylabel("Count")
        ax.set_title("Residuals Histogram", color=TEXT, fontsize=10)
        ax.legend(facecolor=SURFACE, edgecolor=MUTED, labelcolor=TEXT, fontsize=8)
...
with tab_data:
    ...
    di1.metric("Total Samples",   "20,640")
    di2.metric("Features",        "8")
    di3.metric("Missing Values",  "0")
    di4.metric("Target Range",    f"${y.min()*100:.0f}k – ${y.max()*100:.0f}k")
    ...
    with col_c:
        ...
        ax.axvline(y.mean(), color=TEAL, linewidth=1.5, linestyle="--",
                   label=f"Mean = {y.mean():.2f}")
        ax.set_xlabel("Median House Value ($100k)")
        ax.set_ylabel("Count")
        ax.set_title("Distribution of Median House Value", color=TEXT, fontsize=10)
        ax.legend(facecolor=SURFACE, edgecolor=MUTED, labelcolor=TEXT, fontsize=8)
        fig.tight_layout()
        st.pyplot(fig)
        plt.close(fig)
    ...
    with col_d:
        ...
        ax.set_title("Feature vs Target Correlation", color=TEXT, fontsize=10)
        ax.invert_yaxis()
        fig.tight_layout()
        st.pyplot(fig)
        plt.close(fig)
    ...
    ax.set_title("Correlation Matrix (lower triangle)", color=TEXT, fontsize=10, pad=12)
    ...
    st.dataframe(
        X.head(8).style.background_gradient(cmap="Blues", axis=0),
        use_container_width=True,
    )
...
Secondary: MSE, RMSE
    ...
    search_df = pd.DataFrame({
        ...
        "Description":    [
            ...
            "Uniform = equal weight; Distance = closer neighbours weighted more",
    ...
    st.markdown("---")
    st.markdown(
        "<p style='color:#7a97b5; font-size:0.8rem;'>"
        "Dataset: California Housing (sklearn.datasets.fetch_california_housing) · "
        "Source: 1990 U.S. Census · 20,640 block groups</p>",
        unsafe_allow_html=True,
    )
